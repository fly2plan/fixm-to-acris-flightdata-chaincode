...
const mapAttributesOfElement = (elementName: string, elementObj?: any) => {
  ...
  Object.keys(AirMoveAttributes).forEach((key) => {
    ...
    switch (valueType) {
      case "array":
        AirMoveAttributes[key] = jsHandle.fetchDataFromPath(
          FIXM_DATA,
          keyValue
        );
        break;
      case "string":
        if (keyValue.includes("_collection")) {
          ...
          const iterobj = jsHandle.fetchDataFromPath(
            FIXM_DATA,
            collectionObjectDetails.path
          );
          jsHandle.generateCollectionFromObject(
            AirMoveAttributes,
            key,
            collectionObjectDetails,
            iterobj
          );
        } else {
          AirMoveAttributes[key] = mapAttributesOfElement(
