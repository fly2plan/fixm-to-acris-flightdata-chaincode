...
describe("AcrisDataContract", () => {
  ...
  beforeEach(() => {
    ...
    ctx.stub.getState
      .withArgs("1001")
      .resolves(Buffer.from('{"value":"acris asset 1001 value"}'));
    ctx.stub.getState
      .withArgs("1002")
      .resolves(Buffer.from('{"value":"acris asset 1002 value"}'));
  });
  ...
  describe("#createAcrisAsset", () => {
    it("should create a acris asset", async () => {
      ...
      ctx.stub.putState.should.have.been.calledOnceWithExactly(
        "1003",
        Buffer.from('{"value":"acris asset 1003 value"}')
      );
    });

    it("should throw an error for a acris asset that already exists", async () => {
      await contract
        .createAcrisAsset(ctx, "1001", "myvalue")
        .should.be.rejectedWith(/The acris asset 1001 already exists/);
    });
  });
  ...
  describe("#readAcrisAsset", () => {
    it("should return a acris asset", async () => {
      await contract
        .readAcrisAsset(ctx, "1001")
        .should.eventually.deep.equal({ value: "acris asset 1001 value" });
    });

    it("should throw an error for a acris asset that does not exist", async () => {
      await contract
        .readAcrisAsset(ctx, "1003")
        .should.be.rejectedWith(/The acris asset 1003 does not exist/);
    });
  });

  describe("#updateAcrisAsset", () => {
    it("should update a acris asset", async () => {
      await contract.updateAcrisAsset(
        ctx,
        "1001",
        "acris asset 1001 new value"
      );
      ctx.stub.putState.should.have.been.calledOnceWithExactly(
        "1001",
        Buffer.from('{"value":"acris asset 1001 new value"}')
      );
    });

    it("should throw an error for a acris asset that does not exist", async () => {
      await contract
        .updateAcrisAsset(ctx, "1003", "acris asset 1003 new value")
        .should.be.rejectedWith(/The acris asset 1003 does not exist/);
    });
  });
