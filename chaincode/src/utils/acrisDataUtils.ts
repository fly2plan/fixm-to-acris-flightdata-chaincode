...
export class AcrisDataHelper {
  public static getUniqueKey(flightData: object) {
    let flightKey = "";
    try {
      const flightNumber = this.getDataFromAcris(flightData, [
        "flightNumber",
        "trackNumber",
      ]);
      let originDate = this.getDataFromAcris(flightData, ["originDate"]);
      if (originDate === null) {
        originDate = this.getDataFromAcris(flightData, [
          "departure",
          "estimated",
        ]);
      }
      const departureAirport = this.getDataFromAcris(flightData, [
        "departureAirport",
      ]);
      let flightCode = this.getDataFromAcris(flightData, [
        "operatingAirline",
        "iataCode",
      ]);
      if (flightCode === null) {
        flightCode = this.getDataFromAcris(flightData, [
          "operatingAirline",
          "icaoCode",
        ]);
      }
  ...
  public static async iteratorToHistory(
    iterator: Iterators.HistoryQueryIterator
